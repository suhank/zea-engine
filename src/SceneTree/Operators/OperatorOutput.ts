...
import { OperatorOutputMode } from '@SceneTree/Parameters/OperatorOutputMode'
...
class OperatorOutput extends EventEmitter {
   ...
   */
  setParam(param: Parameter, index = -1): void {
    if (this.param) {
      this.param.unbindOperator(this)
    }
    this.param = param
    if (this.param) {
      this._paramBindIndex = this.param.bindOperatorOutput(this, index)
    }
  ...
  setDirty(): void {
    if (this.param) {
      this.param.setDirty(this._paramBindIndex)
    }
  }
  ...
  getValue(): unknown {
    if (this.param) {
      return this.param.getValueFromOp(this._paramBindIndex)
    } else {
  ...
  setClean(value: unknown): void {
    if (this.param) {
      this.param.setCleanFromOp(value, this._paramBindIndex)
    }
  }
  ...
  detach(): void {
    ...
    this.detached = true
    this._paramBindIndex = this.param ? this.param.unbindOperator(this) : -1
  }
  ...
  reattach(): void {
    this.detached = false
    if (this.param) {
      this._paramBindIndex = this.param.bindOperatorOutput(this, this._paramBindIndex)
    }
  }
  ...
  rebind(): void {
    if (this.param) {
      this.param.unbindOperator(this)
      this._paramBindIndex = this.param.bindOperatorOutput(this)
    }
  }
}
