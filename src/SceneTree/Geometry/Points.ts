...
import { Vec3, Box3 } from '@Math/index'
  ...
  loadBin(reader: BinReader): void {
    ...
    if (numVerts < 256) {
      ...
      for (let i = 0; i < numVerts; i++) {
        ...
        if (positions) positions.setValue(i, bboxMat.transformVec3(pos))
      }
    } else {
      ...
      for (let i = 0; i < numClusters; i++) {
        ...
        for (let j = clusters[i]['range'].x; j < clusters[i]['range'].y; j++) {
          ...
          if (positions) positions.setValue(j, bboxMat.transformVec3(pos))
        }
      }
    }
  }
