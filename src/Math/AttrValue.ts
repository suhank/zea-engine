...
abstract class AttrValue {
   ...
   */
  isValid(): boolean {
    for (const v of this.__data) if (v == Infinity || isNaN(v)) return false
   ...
   *
   * @return {TypedArray} - Returns the result as an array.
   */
  asArray(): TypedArray {
    return this.__data
  }
   ...
   */
  abstract toJSON(): Record<string, unknown> | any
   ...
   * @private
   */
  static createFromFloat32Buffer(buffer: ArrayBuffer, offset: number): AttrValue {
   ...
   * @return {AttrValue} - Returns the constructed value.
   */
  static createFromBuffer(buffer: ArrayBuffer, byteOffset: number): AttrValue {
    throw new Error('Not yet implemented for this type:' + this.constructor.name)
  }
