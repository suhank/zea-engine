...
testingHarness.registerTest('System/GPUTest/1024x99904', (domElement, resources) => {
  const start = performance.now();
  gpuTest(domElement, resources, 32, 223, 2.6)
  console.log("Done 'GPUTest_1024x99904':" + (performance.now() - start));
});

testingHarness.registerTest('System/GPUTest/49x1001112', (domElement, resources) => {
  const start = performance.now();
  gpuTest(domElement, resources, 7, 707, 2.6)
  console.log("Done 'GPUTest_49x1001112':" + (performance.now() - start));
});

testingHarness.registerTest('System/GPUTest/484x1001112', (domElement, resources) => {
  const start = performance.now();
