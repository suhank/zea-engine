...
testingHarness.registerTest('Geometry/Meshes/GridCloud', (domElement, resources)=> {
    const Z = ZeaEngine;
    ...
    for(let i=2; i<maxQuadSize; i++) {
        for(let j=i; j<maxQuadSize; j++) {
            let x = Math.pow(2, i);
            let y = Math.pow(2, j);
            quads[x+'-'+y] = new Z.Plane(x*0.04, y*0.04, x, y);
        }
    }
    ...
    for(let i=0; i<numClouds; i++) {
        ...
        material.getParameter('BaseColor').setValue(Z.Color.random());

        let treeItem = new Z.TreeItem('Tree'+i);
        treeItem.setLocalXfo(new Z.Xfo(new Z.Vec3(i*15+((Math.random()-0.5) * 20), (Math.random()-0.5) * 10, (Math.random()-0.5) * 10)));
