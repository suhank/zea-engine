...
Use NPM to install a simple static HTTP file server.
...
Now, you need to add the engine as a dependency of your project. Invoke the following in the shell.
```
npm install @zeainc/zea-engine
```

This adds zea-engine to the package.json file, and then downloads the engine and puts it into a folder called node_modules.
...
> Note: The download archive above does not include the node_modules folder that is generated by NPM. After downloading and extracting the archive, you will need to run 'npm install' in the folder to cause NPM to created the node_modules folder, and download the engine package specified in the package.json file.
