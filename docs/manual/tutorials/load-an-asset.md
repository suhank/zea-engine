...
In this tutorial we will show how to load a simple OBJ model. For each type of asset, the correct [Asset](api/SceneTree/AssetItem?id=assetitem) class must be constructed and used to load the file.
...
The following code loads an an OBJ file into an object of type [ObjAsset](api/SceneTree/ObjAsset). Loading an asset is an asynchronous operation, we can create a call back like below:
